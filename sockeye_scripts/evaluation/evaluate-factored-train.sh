...
# calling line: ./sockeye_scripts/evaluation/evaluate-factored.sh processed_datasets/de-text-noisy-durations0.1-en-phones-durations/train.en models/sockeye/baseline_factored_noised0.1/train_eval_checkpoint_16/train.en.output

#ROOT=/content/Arabic-Dubbing
#DATA_HOME=${ROOT}/processed_datasets
#MODELS_HOME=/content/drive/MyDrive/models/sockeye

REF=$1 #processed_datasets/de-text-noisy-durations0.1-en-phones-durations/train.en
# English phonemes with durations from the dataset not the model output 

HYP=$2 #models/sockeye/trained_baselines/baseline_factored_noised0.1/train_eval_checkpoint_16/train.en.output
...
if [[ ! -s ${DATA_HOME}/ar-text-without-durations-en-text-without-durations/train.en ]]; then
    ...
    # REF_TEXT: english words without bpe (train dataset)
    # SRC_TEXT: arabic words without bpe (train dataset)

    REF_TEXT=${DATA_HOME}/ar-text-without-durations-en-text-without-durations/train.debpe.en 
    SRC_TEXT=${DATA_HOME}/ar-text-without-durations-en-text-without-durations/train.debpe.ar
    if [[ ! -s ${REF_TEXT} ]]; then
        sed -r 's/(@@ )|(@@ ?$)//g' ${DATA_HOME}/ar-text-without-durations-en-text-without-durations/train.en > ${REF_TEXT}
        sed -r 's/(@@ )|(@@ ?$)//g' ${DATA_HOME}/ar-text-without-durations-en-text-without-durations/train.ar > ${SRC_TEXT}
    fi
fi
